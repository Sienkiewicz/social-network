...
const messagesReducer = (state: InitialStateMessagesType = initialState, action: ActionTypes):InitialStateMessagesType => {
	switch (action.type) {
		case 'ADD_MESSAGE':
			return {
				...state,
...
export const actions={ 
	addMessageActionCreator: (textOfNewMessage: string) => ({ type: 'ADD_MESSAGE', textOfNewMessage } as const)
}
